...
resource "aws_route53_record" "cert_validation" {
  ...
  name     = aws_acm_certificate.domain_cert[each.value[0]].domain_validation_options[each.value[1]].resource_record_name
  type     = aws_acm_certificate.domain_cert[each.value[0]].domain_validation_options[each.value[1]].resource_record_type
  zone_id  = data.aws_route53_zone.primary.id
  records  = [aws_acm_certificate.domain_cert[each.value[0]].domain_validation_options[each.value[1]].resource_record_value]
  ttl      = 60
}
